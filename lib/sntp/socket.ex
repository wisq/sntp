defmodule SNTP.Socket do
  ...
  """
  @spec send(t()) :: t()
  def send(%__MODULE__{port: nil} = socket),
    do: add_error(socket, :no_port_open, "Unable to send NTP message")
  ...
  def send(
    ...
    t3s = div(ts, 1000) + 2_208_988_800
    t3f = Kernel.round(Integer.mod(ts, 1000) / 1000 * 4_294_967_296)
    ...
    case :gen_udp.send(
      ...
      {:error, reason} ->
        add_error(socket, :udp_send, reason)
          ...
          {:udp, ^port, ip, ^host_port, data} ->
            message =
              Kernel.struct(message,
                ...
                ip: ip,
                sent_at: ts,
                received_at: :erlang.system_time(1000)
              )
  ...
  defp validate_opts(opts) do
    opts
    |> Enum.reduce(%{}, fn
      {k, v}, acc when is_atom(k) and k in @keys ->
        Map.update(acc, k, v, &(&1 = v))

      {k, v}, acc when is_binary(k) and k in @keys ->
        Map.update(acc, String.to_atom(k), v, &(&1 = v))

      {k, v}, acc when is_list(k) and k in @keys ->
        Map.update(acc, List.to_atom(k), v, &(&1 = v))

      _, acc ->
        acc
    end)
    |> Enum.reduce(%{}, fn
      {:host, v}, acc when is_binary(v) or is_list(v) ->
        Map.put(acc, :host, '#{v}')

      {:host, v}, acc when not is_binary(v) or is_list(v) ->
        Map.put(acc, :host, 'pool.ntp.org')

      {:host_port, v}, acc when v not in 0..65535 ->
        Map.put(acc, :host_port, 123)

      {:resolve_reference, v}, acc when not is_boolean(v) ->
        Map.put(acc, :resolve_reference, false)
